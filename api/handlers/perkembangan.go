	...
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			if pgErr, ok := err.(*pgconn.PgError); ok && pgErr.Code == "23503" {
				if pgErr.ConstraintName == "perkembangan_id_anak_fkey" { // Ganti dg nama constraint yg benar
...
func UpdatePerkembanganHandler(dbpool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Kader ID tidak diperlukan untuk update data perkembangan itu sendiri
		// kaderIdInterface, exists := c.Get("kaderId")
...
func DeletePerkembanganHandler(dbpool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			// Generally, deleting a perkembangan record shouldn't cause FK violations unless other tables depend on it.
