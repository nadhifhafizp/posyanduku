	...
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			if pgErr, ok := err.(*pgconn.PgError); ok && pgErr.Code == "23505" {
				// Cek constraint unik NIK
				if pgErr.ConstraintName == "ibu_nik_key" { // Pastikan nama constraint sesuai dengan database Anda
...
func GetIbuSimpleHandler(dbpool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		var daftarIbu []models.IbuOption // Menggunakan IbuOption
...
func GetIbuByIdHandler(dbpool *pgxpool.Pool) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			// Periksa apakah error karena tidak ada baris (NotFound) atau error lain
			if err.Error() == "no rows in result set" { // Atau gunakan errors.Is(err, pgx.ErrNoRows) jika pakai pgx/v5
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			if pgErr, ok := err.(*pgconn.PgError); ok && pgErr.Code == "23503" {
				// Cek constraint foreign key (misal, jika terhubung ke tabel 'anak')
				if pgErr.ConstraintName == "anak_id_ibu_fkey" { // Ganti dengan nama constraint yang benar
